...
/**
 * The sky. A watercolour wash generated by the p5.brush mark factory, mapped
 * onto a plane far behind everything.
 *
 * It is unlit and greyscale on purpose. The composite shader bands luminance
 * into screentone, so the wash's soft variation comes out as ORGANIC tone
 * density — drifting clouds of dots rather than the flat empty paper the
 * backgrounds had before. Greyscale also keeps its saturation at zero, so the
 * accent mask correctly ignores it.
 */
function Backdrop({ name }: { name: string }) {
  const url = plateUrl(name)
  if (!url) return null
  return <BackdropPlane url={url} />
}
...
function DeskScene({ mats, closing = false }: { mats: Mats; closing?: boolean }) {
  return (
    <>
      ...
      <mesh position={[1.15, -0.28, 0.35]} material={mats.accent}>
  ...
  return (
    <>
      ...
      <group position={[2.0, -0.7, 0]} rotation={[0, -0.25, 0]}>
        ...
        <mesh position={[0.55, 0.08, 0.4]} material={mats.accent}>
      ...
      return (
        <group>
          ...
          {[0.34, 0.52, 0.7].map((r, i) => (
            <mesh key={r} position={[0, 0.1, 0.1 + i * 0.06]} material={mats.accent}>
    ...
    case 'verdant':
      return (
        <group>
          ...
          <mesh position={[0.5, -0.5, 0.2]} rotation={[0, 0, 0.6]} material={mats.accent}>
    ...
    case 'cloudsense':
      return (
        <group>
          ...
          <mesh position={[0.1, -0.72, 0]} rotation={[0, 0, -0.35]} material={mats.accent}>
    ...
    default:
      return (
        <group>
          ...
          <mesh position={[0.1, 0.75, 0]} rotation={[0, -0.4, 0]} material={mats.accent}>
...
function TreeScene({ mats }: { mats: Mats }) {
  ...
  const cluster = (crimson: boolean) => (
    <>
      <mesh material={crimson ? mats.accent : mats.paper}>
        <sphereGeometry args={[0.5, 16, 12]} />
      </mesh>
      <mesh position={[0.42, 0.16, 0.1]} material={crimson ? mats.accent : mats.paper}>
        <sphereGeometry args={[0.34, 14, 10]} />
      </mesh>
      <mesh position={[-0.36, 0.2, -0.08]} material={crimson ? mats.accent : mats.tone}>
